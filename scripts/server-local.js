...
// PeopleGames analytics (matches Vercel /api/peoplegames/analytics)
app.get('/api/peoplegames/analytics', async (_req, res) => {
  try {
    const SUPABASE_URL = getEnv('SUPABASE_URL') || getEnv('VITE_SUPABASE_URL')
    const SUPABASE_KEY = getEnv('SUPABASE_ANON_KEY') || getEnv('SUPABASE_KEY') || getEnv('VITE_SUPABASE_ANON_KEY')
    if (!SUPABASE_URL || !SUPABASE_KEY) {
    ...
    ;(wrongEvents || []).forEach((e) => {
      ...
      if (id) wrongChoices[id] = (wrongChoices[id] || 0) + 1
    })
...
// Endpoint для озвучки: gTTS (приоритет) → Edge TTS (Светлана) → Python edge-tts
...
/** Лог ошибки TTS с полными деталями (message, code, stack, прочие поля) */
...
app.post('/api/tts', async (req, res) => {
  ...
  // 2) edge-tts-node (WebSocket, Светлана)
  let tts
  try {
    tts = new MsEdgeTTS({})
