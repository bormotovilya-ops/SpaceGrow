...
Озвучка текста голосом. Приоритет: **gTTS** (Google Translate, без ключей) → **Edge TTS** (Светлана) → **Web Speech API** (в браузере).

- **gTTS** — HTTP, без регистрации, работает на Vercel.
- **Edge TTS** — Светлана, на Vercel WebSocket может блокироваться.
- **Web Speech API** — встроенный голос браузера, fallback при ошибке API.
