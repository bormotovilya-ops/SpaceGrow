...
const stages = [
  ...
  },
  {
    id: 'tripwire',
    name: 'Трипвайер',
    subtitle: 'первая денежная транзакция',
    image: '/images/3-5.png',
    questions: [
      {
        id: 'tripwire_1',
        text: 'Есть ли у вас трипвайер (первая импульсивная покупка) после лидмагнита?'
      },
      {
        id: 'tripwire_2',
        text: 'Насколько трипвайер логично вытекает из лидмагнита?'
      },
      {
        id: 'tripwire_3',
        text: 'Конвертируется ли трипвайер в первую покупку стабильно?'
      }
    ]
  },
  {
...
const getAnswerOptions = (questionId) => {
  ...
  const baseOptions = {
    ...
    tripwire_1: [
      { value: 0, label: '❌ Нет трипвайера' },
      { value: 25, label: '⚠️ Есть, но не работает' },
      { value: 50, label: '✓ Есть, работает частично' },
      { value: 100, label: '✅ Есть и конвертирует стабильно' }
    ],
    tripwire_2: [
      { value: 0, label: '❌ Нет связи с лидмагнитом' },
      { value: 25, label: '⚠️ Связь слабая' },
      { value: 50, label: '✓ Логично вытекает' },
      { value: 100, label: '✅ Естественное продолжение' }
    ],
    tripwire_3: [
      { value: 0, label: '❌ Не конвертируется' },
      { value: 25, label: '⚠️ Конвертируется редко' },
      { value: 50, label: '✓ Конвертируется периодически' },
      { value: 100, label: '✅ Стабильная конверсия в первую покупку' }
    ],
    autofunnel_1: [
